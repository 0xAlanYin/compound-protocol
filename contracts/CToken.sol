...
abstract contract CToken is CTokenInterface, ExponentialNoError, TokenErrorReporter {
     ...
     * @param comptroller_ The address of the Comptroller   审计合约的地址
     * @param interestRateModel_ The address of the interest rate model   利率模型的地址
     * @param initialExchangeRateMantissa_ The initial exchange rate, scaled by 1e18   初始汇率，乘以 1e18
     * @param name_ EIP-20 name of this token  EIP-20 名称
     * @param symbol_ EIP-20 symbol of this token  EIP-20 符号
     * @param decimals_ EIP-20 decimal precision of this token  EIP-20 小数精度
     */
    function initialize(
     ...
     */
    // 将应计利息计入总借款和准备金
    // 这将计算从最后一个检查点区块到当前区块所累积的利息，并将新的检查点写入存储。
    function accrueInterest() public virtual override returns (uint256) {
        /* Remember the initial block number */
        uint256 currentBlockNumber = getBlockNumber(); // 获取当前区块高度
        uint256 accrualBlockNumberPrior = accrualBlockNumber; // 获取上一次计算利息的区块高度

        /* Short-circuit accumulating 0 interest */
        // 如果当前区块高度与上一次计算利息的区块高度一致，则直接返回，不需要计算
        if (accrualBlockNumberPrior == currentBlockNumber) {
            return NO_ERROR;
        }

        /* Read the previous values out of storage */
        uint256 cashPrior = getCashPrior(); // 获取当前现金余额
        uint256 borrowsPrior = totalBorrows; // 获取当前总借款
        uint256 reservesPrior = totalReserves; // 获取总准备金
        uint256 borrowIndexPrior = borrowIndex; // 获取借款指数

        /* Calculate the current borrow interest rate */
        // 计算当前借款利率（这里支持不同的利率模型）
        uint256 borrowRateMantissa = interestRateModel.getBorrowRate(cashPrior, borrowsPrior, reservesPrior);
        require(borrowRateMantissa <= borrowRateMaxMantissa, "borrow rate is absurdly high");

        /* Calculate the number of blocks elapsed since the last accrual */
        uint256 blockDelta = currentBlockNumber - accrualBlockNumberPrior; // 计算当前区块与上一次计算利息的区块高度差，这是还没有计算的利息区块数
        ...
        Exp memory simpleInterestFactor = mul_(Exp({mantissa: borrowRateMantissa}), blockDelta); // 区块区间内的单位利息
        uint256 interestAccumulated = mul_ScalarTruncate(simpleInterestFactor, borrowsPrior); // 计算区块区间内的总利息
        uint256 totalBorrowsNew = interestAccumulated + borrowsPrior;   // 将总利息加到总借款上
        uint256 totalReservesNew =
            mul_ScalarTruncateAddUInt(Exp({mantissa: reserveFactorMantissa}), interestAccumulated, reservesPrior); // 将总利息乘以储备金率，再加上总储备金
        uint256 borrowIndexNew = mul_ScalarTruncateAddUInt(simpleInterestFactor, borrowIndexPrior, borrowIndexPrior); // 将单位利息乘以借款指数，再加上借款指数
    ...
    function mintFresh(address minter, uint256 mintAmount) internal {
        ...
        // 审计：使用 comptroller 合约的 mintAllowed 函数检查 mint 操作是否被允许
        ...
        // 检查当前区块高度是否与最后一次计算利息的区块高度一致：因为不一致，说明有新的利息产生，需要重新计算
         ...
         */
        // 计算 mintTokens 数量：mintTokens = actualMintAmount / exchangeRate
         ...
         */
        // 更新总供应量和用户持有的 cToken 数量
        totalSupply = totalSupply + mintTokens;
        accountTokens[minter] = accountTokens[minter] + mintTokens;

        /* We emit a Mint event, and a Transfer event */
        // 触发 Mint 事件和 Transfer 事件
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    // 使用 _setInterestRateModel 积累利息并更新利率模型
    ...
    /**
     * @notice Gets balance of this contract in terms of the underlying 获取此合约标的代币的余额
     * @dev This excludes the value of the current message, if any      这排除了当前消息的任何价值（如果有）
     * @return The quantity of underlying owned by this contract        此合约拥有的基础代币数量
     */
