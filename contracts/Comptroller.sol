 ...
 * @author Compound
 */
// Comptroller 审计合约：主要对存款、取款、借款、还款、清算等核心操作进行审计和校验
// - xxxVerify 没做任何实质性的操作
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     */
    // 添加资产包含在账户流动性计算中
    // - enterMarkets 可以将指定的多个 cToken 作为抵押品，对应地就可增加用户的可借额度
     ...
     */
    // 从 sender 账户中移除流动性计算的资产
    // - exitMarket 则可以将指定 cToken 从抵押品列表中移除，但是，如果用户存在借款时，exitMarket 的资产价值不能超过借款价值。
    function exitMarket(address cTokenAddress) external override returns (uint256) {
        ...
        // 常规的优化思路：将数据加载到内存中，然后遍历，找到要删除的索引，然后与最后一个元素交换位置，然后删除最后一个元素
     ...
     *             account liquidity in excess of collateral requirements,   账户流动性超过抵押品要求
     *          account shortfall below collateral requirements)             账户短缺低于抵押品要求
     */
    // 检测当前账户的流动性抵押品要求
    function getAccountLiquidity(address account) public view returns (uint256, uint256, uint256) {
        // liquidity 表示剩余的可借额度，它是用户的抵押物总价值（乘以抵押因子后的价值）sumCollateral 和总债务 sumBorrowPlusEffects 的差额
        //当这个差额为正数时，则 liquidity = sumCollateral - sumBorrowPlusEffects，此时 shortfall 为 0；
        //而差额为负数时，则 liquidity 为 0，shortfall = sumBorrowPlusEffects - sumCollateral，此时表示用户债务已经超过了清算门槛，已经可以被清算。
    ...
    function _setCollateralFactor(CToken cToken, uint256 newCollateralFactorMantissa) external returns (uint256) {
        ...
        // 如果清算因子不为 0，并且该 cToken 对应的标的资产价格为0，则设置失败
